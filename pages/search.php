...
<head>
    ...
    <title>Search - GETE Yejebena Bunna</title>
</head>
<?php 
    $q = $_GET['q'];
    $length = 0;
    if(!empty($q)){
        ...
        $coffees = array();
        $query = mysqli_query($con, "Select * from coffee where name LIKE '%".$q."%'") or die(mysqli_error($con)); //Query the users table
        $length = $exists = mysqli_num_rows($query);
...
<body>
    ...
    <div class="coffees-con">
        ...
        <?php
            if($length > 0) echo "<h4>Found $length results for \"$q\"</h4>"; else echo "<h4>Found no results for \"$q\"</h4>";
